...
export class AddCustomOrder1766751238059 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "order_items" ADD "name" character varying(255)`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."orders_payment_status_enum" AS ENUM('unpaid', 'paid')`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "payment_status" "public"."orders_payment_status_enum" NOT NULL DEFAULT 'unpaid'`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."orders_order_type_enum" AS ENUM('catalog', 'custom')`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "order_type" "public"."orders_order_type_enum" NOT NULL DEFAULT 'catalog'`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "tracking_number" character varying(128)`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "orders" ADD "shipping_cost" numeric(10,2) NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(

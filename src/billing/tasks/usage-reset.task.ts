...
export class UsageResetTask {
  ...
  async handleUsageReset() {
    ...
    const overdueSubscriptions = await this.userSubscriptionRepository.createQueryBuilder('sub')
      .where('sub.status = :status', { status: 'active' })
      .andWhere('sub.current_period_end <= :now', { now })
      .getMany();

    this.logger.log(`Found ${overdueSubscriptions.length} subscriptions to reset.`);
    ...
    for (const sub of overdueSubscriptions) {
      try {
        ...
        this.logger.log(`Reset usage and updated period for User ${sub.userId}`);
      } catch (err) {
